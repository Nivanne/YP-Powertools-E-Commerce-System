...
session_start();
	...
	if(isset($_POST['submit'])){
		...
		if(empty($name) || empty($email) || empty($password) || empty($confirmPassword)){
			header("Location: ../superadmin_register.php?signup=empty");
			exit();
		} else {
			...
			if(!preg_match("/^[a-zA-Z]*$/", $name)){
				header("Location: ../superadmin_register.php?signup=invalid");
				exit();
			} else if (!filter_var($email, FILTER_VALIDATE_EMAIL)){
				header("Location: ../superadmin_register.php?signup=invalid");
				exit();
			} else if ($password != $confirmPassword){
				header("Location: ../superadmin_register.php?signup=password_not_same");
				exit();
			} else {
				...
				if($resultCheck > 0){
					header("Location: ../superadmin_register.php?signup=usertaken");
					exit();
				} else {
					//Hash password
					...
					$sql = "INSERT INTO adminlogin (name,email,password,profile_pic,RegisteredDate) VALUES ('$name','$email', '$hashedPassword','$fileName',NOW());";

					//Execute Query
					mysqli_query($Database, $sql);

					//Redirect
					header("Location: ../superadmin_register.php?signup=success");
					exit();
		...
		header("Location: ../superadmin_register.php?signup=used_get");
		exit();
	}
?>